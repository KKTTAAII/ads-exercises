...
describe("pop", function () {
  it("removes node at end and decrements length", function () {
    let lst = new LinkedList([5, 10, 20, 30]);

    expect(lst.pop()).toBe(30);
    expect(lst.head.val).toBe(5);
    expect(lst.tail.val).toBe(20);
    expect(lst.length).toBe(3);

    expect(lst.pop()).toBe(20);
    expect(lst.head.val).toBe(5);
    expect(lst.tail.val).toBe(10);
    expect(lst.length).toBe(2);
  ...
  it("removes the only last node", function () {
  ...
  it("throws error when no Node", function () {
    let lst = new LinkedList();
    try {
      lst.pop();
    } catch (e) {
      expect(e).toBe("list is empty");
    }
  });
});
...
describe("getAt", function () {
  it("gets val at index", function () {
    let lst = new LinkedList([5, 10, 12, 13, 15]);

    expect(lst.getAt(0)).toBe(5);
    expect(lst.getAt(1)).toBe(10);
    expect(lst.getAt(2)).toBe(12);
    expect(lst.getAt(3)).toBe(13);
    expect(lst.getAt(4)).toBe(15);
  });
  ...
  it("throws error when there is no index found", function () {
    let lst = new LinkedList([5, 10]);

    try {
      lst.getAt(5);
    } catch (e) {
      expect(e).toBe("index not found");
    }
  });
});
...
describe("setAt", function () {
  it("sets val at index", function () {
    let lst = new LinkedList([5, 10, 25]);

    expect(lst.setAt(0, 1));
    expect(lst.setAt(1, 2));
    expect(lst.setAt(2, 20));
...
describe("insertAt", function () {
  it("inserts node and adjusts nearby nodes", function () {
    ...
    expect(lst.head.next.next.next.val).toBe(13);
  });
  ...
  it("throws error when no index is found", function () {
...
describe("removeAt", function () {
  ...
  it("removes in-between node from list", function () {
    let lst = new LinkedList(["a", "b", "c"]);
